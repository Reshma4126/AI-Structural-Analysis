...
export default function XAIDashboardPage() {
  ...
  useEffect(() => {
    const loadXaiData = async () => {
      ...
      try {
        ...
        setBeamData(res);
        if (res?.beam?.geometry) {
          if (res.beam.geometry.depth) setDepth(res.beam.geometry.depth);
          if (res.beam.geometry.length) setSpanLength(res.beam.geometry.length / 1000);
        }
      } catch (err) {
        console.warn("Using fallback XAI beam parameters:", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  const simulatedSafetyFactor = (estimatedMn / (estimatedMu || 1)).toFixed(2);

  const targetBeam = beamData?.beam || {};
      ...
      <div className="space-y-6 max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded border border-concrete-300 shadow-blueprint">
          <div>
            ...
            <p className="text-xs text-navy-500 mt-1 font-mono">
              Unpack structural AI predictions for <strong className="text-navy-900">{targetBeam.name || 'Beam B-104'}</strong> via SHAP attributions and parameter sensitivity curves.
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white p-6 rounded border border-concrete-300 shadow-blueprint space-y-4">
            <div className="flex items-center justify-between border-b border-concrete-200 pb-3">
              <div>
                ...
                <p className="text-xs text-navy-500">Contribution of each design variable to Beam Health & Safety Factor</p>
              </div>
            ...
            <div className="p-3 bg-concrete-50 rounded border border-concrete-200 text-xs text-navy-700 leading-relaxed font-body">
              <strong className="font-semibold text-navy-900">Key Insight:</strong> <span className="font-mono text-steel-700">Span Length (L = {spanLength}m)</span> and <span className="font-mono text-steel-700 font-bold">Live Load (Wl = {liveLoad} kN/m)</span> have the strongest positive contribution to bending demand, while <span className="font-mono text-cyanAccent-700">Steel Yield Strength (Fy = 500 MPa)</span> provides the primary capacity offset.
            </div>
          </div>
          ...
          <div className="bg-white p-6 rounded border border-concrete-300 shadow-blueprint space-y-4">
            ...
            <div className="p-3 bg-concrete-50 rounded border border-concrete-200 text-xs text-navy-700 leading-relaxed font-body">
              <strong className="font-semibold text-navy-900">Trade-off Analysis:</strong> Increasing depth past 540mm yields diminishing returns on safety factor while increasing structural dead weight.
            </div>
          </div>
        </div>
